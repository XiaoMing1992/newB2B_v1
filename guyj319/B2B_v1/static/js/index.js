...
$("#loginForm").validate({
    ...
    errorPlacement: function(error, element){
      switch(element.attr("name")){
        case "loginUserName":
          $("#loginUserNameErrorTip").text(error.text());
          break;
        case "loginPassword":
          $("#loginPasswordErrorTip").text(error.text());
          break;
      }
    },
    messages: {
...
$("#invitationForm").validate({
    ...
    errorPlacement: function(error, element){
      $("#invitationCodeTip").text(error.text());
    },
    messages: {
        invitationCode:{
          required: "请输入邀请码!",
          maxlength: "邀请码不能多于4个字符",
          minlength: "邀请码不能少于4个字符" }
    },
    submitHandler: function() {
...
$("#registerForm").validate({
    ...
    messages: {
        ...
        verifyCode: { required: "请输入手机验证码",
                      minlength: "验证码长度不少于4个字符",
                      maxlength: "验证码长度不多于4个字符"},
