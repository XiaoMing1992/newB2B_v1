...
from myadmin.models import activity_table, login_error_table
import datetime
from myadmin.AdminLoginStatus import AdminLoginStatus
...
class Login(object):
    ...
    def login(self, user_ip, user_name, password):
          ...
          try:
             ...
             if error.error_times > 5 and forbid_end_time > now_time: #验证无效次数连续大于5次，并且1小时还没有过去，该ip仍然处于受禁状态
                 return AdminLoginStatus.ERROR_EXCEED  #锁住IP，返回404
                ...
                return AdminLoginStatus.SERVER_ERROR

          admin_check = Admin_check() #实例化
          user = admin_check.authenticate(user_name, password) #调用其验证函数验证用户是否输入正确
          if user is not None:
                return user.id  #返回该管理员的id
                #return 666     #验证成功
          else:#验证无效
                try:
                     error = login_error_table.objects.get(IP=user_ip) #获取记录
    ...
    def activity_log(self, admin_id, user_ip):
        '''
        :param admin_id:
        :param user_ip:
        :return:
        ...
        try:
            ...
            temp_user = activity_table.objects.create(admin_id=admin_id,
